...
function migrateLegacyJson(database) {
  ...
  const insert = database.prepare(`
    INSERT INTO products (
      ...
      tags, price, brand, stock, description, image
    ) VALUES (
      @model, @type, @goods_type, @color, @body_material, @handle_material,
      @body_weight, @package_weight, @cartridge_size, @cartridge_nut_material,
      @left_handed_nut, @hot_cold_output, @package_dimensions, @postal_hose,
 ...
 */
export async function sqliteFulfillOrder(items) {
  const database = await getSqliteDbAsync();

  const deduct = database.transaction((orderItems) => {
    ...
    for (const [productIdStr, quantity] of Object.entries(required)) {
      ...
      const row = database.prepare('SELECT id, stock, model FROM products WHERE id = ?').get(productId);
      if (!row) throw new Error(`Product not found: ${productId}`);
      if (row.stock < quantity) {
        throw new Error(`Insufficient stock for product ${productId} (${row.model})`);
      }
    }

    for (const [productIdStr, quantity] of Object.entries(required)) {
      const productId = Number(productIdStr);
      database.prepare('UPDATE products SET stock = stock - ? WHERE id = ?').run(quantity, productId);
    }
    ...
    return database
      ...
      .all(...ids)
      .map((row) => sqliteRowToProduct(row));
  });
...
/**
 * @param {number} id
 * @param {{ mode: 'set' | 'delta'; value: number }} adjustment
 */
export async function sqliteAdjustProductStock(id, adjustment) {
  const database = await getSqliteDbAsync();
  const existing = database.prepare('SELECT * FROM products WHERE id = ?').get(id);
  if (!existing) throw new Error('Product not found');
  ...
  database.prepare('UPDATE products SET stock = ? WHERE id = ?').run(nextStock, id);
  return sqliteRowToProduct(
    database.prepare('SELECT * FROM products WHERE id = ?').get(id),
  );
}
