...
export async function bulkAdjustPrices(percent, { goodsType } = {}) {
  ...
  for (const p of products) {
    ...
    const calculatedPrice = Math.max(0, Math.round(p.price * factor));

    // Round the final price up to the next 50,000 تومان increment.
    const newPrice = Math.ceil(calculatedPrice / 50000) * 50000;
